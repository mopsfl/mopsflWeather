...
export default {
    ...
    ConvertIsoToReadableTime(isoTime: string) {
    ...
    TimeUntil(timestamp: number, timezone = 0, toUnix?: boolean) {
        ...
        if (difference > 0) {
            if (hours > 0) {
                return Strings.ToString(Strings.Languages[App.client.language].WEATHER_INFO_TIMEIN_HOURS, hours);
            } else if (minutes > 0) {
                return Strings.ToString(Strings.Languages[App.client.language].WEATHER_INFO_TIMEIN_MINUTES, minutes);
            } else {
                return Strings.ToString(Strings.Languages[App.client.language].WEATHER_INFO_TIMEIN_SECONDS, seconds);
            }
        } else if (difference < 0) {
            if (hours > 0) {
                return Strings.ToString(Strings.Languages[App.client.language].WEATHER_INFO_TIMEAGO_HOURS, hours);
            } else if (minutes > 0) {
                return Strings.ToString(Strings.Languages[App.client.language].WEATHER_INFO_TIMEAGO_MINUTES, minutes);
            } else {
                return Strings.ToString(Strings.Languages[App.client.language].WEATHER_INFO_TIMEAGO_SECONDS, seconds);
            }
        }
    },
}